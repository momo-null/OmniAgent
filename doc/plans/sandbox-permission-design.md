...
>
> **⚠️ 2026-09-24 变更（与 `capability-unit-refactor-2026-09-24.md` 对齐）**：命令执行（`shell_exec`）已改为**内核 builtin 默认开**。故本条的「默认关闭」前提**被替换**为「默认开 + S2 审批门（危险动作要人点头）+ `full_access` 绕过」。L0 的路径边界对命令执行仍是**绊线**（非硬边界），能同时约束文件工具与命令执行的硬边界仍只有 **S3（OS 级隔离）**。
